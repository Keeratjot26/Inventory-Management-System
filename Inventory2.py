...
import sys
from sklearn.linear_model import LinearRegression   # <-- for regression in Forecast tab

# quick debug to surface Python env early in logs
st.set_page_config(page_title="Smart Inventory Analytics",
                   layout="wide",
                   initial_sidebar_state="expanded")
st.write("Environment: Streamlit startup - Python", sys.version.split()[0])

# ---------------- CONFIG ----------------
# Use Streamlit secrets instead of hardcoded URI
...
st.markdown(
    ...
    [data-testid="stTab"] button div p {
...
def load_mongo_data():
    """Loads products and sales collections, merges them and returns dataframe."""
    try:
        # Use secure client config + ping
        client = MongoClient(
            MONGO_URI,
            tlsAllowInvalidCertificates=False,
            retryWrites=True,
            serverSelectionTimeoutMS=20000,  # 20 sec
            connectTimeoutMS=20000,          # 20 sec
            socketTimeoutMS=20000            # 20 sec
        )
        # Force a connection attempt (ping)
...
if df is None or df.empty:
    ...
    df = pd.DataFrame(columns=[
        'product', 'quantity', 'selling_price', 'purchase_cost',
        'stock_level', 'invoicedate', 'total_sales', 'total_profit'
    ])
...
if date_range is not None and 'invoicedate' in df_filtered.columns:
    ...
    df_filtered = df_filtered[
        (df_filtered['invoicedate'] >= pd.to_datetime(start)) &
        (df_filtered['invoicedate'] <= pd.to_datetime(end))
    ]
...
# ---------------- TABS ----------------
tab_dashboard, tab_forecast, tab_alerts, tab_admin = st.tabs(
    ["📊 Dashboard", "🔮 Forecast", "⚠️ Alerts", "⚙️ Admin"]
)
...
with tab_dashboard:
    ...
    products_at_risk = df_filtered[
        df_filtered['stock_level'] <= df_filtered.get('quantity', 0)
    ].shape[0]

    k1, k2, k3, k4 = st.columns([1.8, 1.8, 1.2, 1.2])
    k1.markdown(
        f'<div class="kpi"><div class="kpi-label">Total Revenue</div>'
        f'<div class="kpi-value">₹{total_revenue:,.0f}</div></div>',
        unsafe_allow_html=True
    )
    k2.markdown(
        f'<div class="kpi"><div class="kpi-label">Total Profit</div>'
        f'<div class="kpi-value">₹{total_profit:,.0f}</div></div>',
        unsafe_allow_html=True
    )
    itv_display = f"{inventory_turnover:.2f}" if not np.isnan(inventory_turnover) else "N/A"
    k3.markdown(
        f'<div class="kpi"><div class="kpi-label">Inventory Turnover</div>'
        f'<div class="kpi-value">{itv_display}</div></div>',
        unsafe_allow_html=True
    )
    k4.markdown(
        f'<div class="kpi"><div class="kpi-label">Products at Risk</div>'
        f'<div class="kpi-value">{products_at_risk}</div></div>',
        unsafe_allow_html=True
    )
    ...
    with left:
        ...
        if 'invoicedate' in df_filtered.columns:
            monthly = (
                ...
                .groupby(df_filtered['invoicedate'].dt.to_period('M'))['quantity']
                .sum()
                .reset_index()
            )
        ...
        # ---------- TOP 30 PRODUCTS HEATMAP (NORMALISED) ----------
        st.markdown('<div class="card" style="margin-top:18px">', unsafe_allow_html=True)
        st.subheader("Profitability Heatmap (Top 30 Products, Avg Daily Profit)")
        ...
        if 'invoicedate' in df_filtered.columns and 'product' in df_filtered.columns:
            ...
            # normalise by days in month
            df_heat['month_days'] = df_heat['invoicedate'].dt.days_in_month
            df_heat['month_days'].replace(0, np.nan, inplace=True)
            df_heat['profit_per_day'] = df_heat['total_profit'] / df_heat['month_days']

            # pick top 30 products by overall profit
            prod_profit = (
                df_heat.groupby('product')['total_profit']
                .sum()
                .abs()
            ...
            # pivot on profit_per_day (mean per month)
            heat = df_heat_top.pivot_table(
            ...
            if not heat.empty:
                fig2, ax2 = plt.subplots(
                    figsize=(10, max(3, min(10, heat.shape[0] * 0.35)))
                )
    ...
    with right:
        ...
        if 'invoicedate' in df_filtered.columns:
            ...
            if not df_exp.empty:
                df_exp['expiry_date'] = df_exp['invoicedate'] + pd.to_timedelta(
                    shelf_life_days, unit='D'
                )
                ...
                if df_exp.empty:
                    ...
                    for _, r in alerts.iterrows():
                        ...
                        st.markdown(
                            f"**{product_name}** — expiry: {expiry_str} "
                            f"(*{days} days from selected date*) &nbsp; {badge}",
                            unsafe_allow_html=True
                        )
            else:
        ...
        # --- Inventory Levels (clean, name-first, categorical x-axis) ---
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Inventory Levels")

        # 1) Pick the best display column: prefer product name over codes/IDs
        ...
        if display_col and 'stock_level' in df_filtered.columns:
            # 2) Build a friendly label column for plotting
            inv = (df_filtered
                   .groupby(display_col, dropna=False)['stock_level']
                   .sum()
                   .reset_index()
                   .rename(columns={display_col: 'display_name'}))

            # Make sure x-axis is treated as categories, not numbers
            ...
            if not inv.empty:
                ...
                # nicer reading: largest first, readable text
                ...
                # Optional: quick table below for exact values
                st.caption("Top 10 products by total stock")
                st.dataframe(
                    inv.rename(columns={'display_name': 'Product', 'stock_level': 'Stock Level'}),
                    use_container_width=True
                )
            else:
                st.info("No stock data to plot after filtering.")
        else:
            st.info("Need product names/codes and 'stock_level' to show inventory levels.")
        st.markdown("</div>", unsafe_allow_html=True)
...
with tab_forecast:
    ...
    if 'invoicedate' in df.columns:
        ...
        if not base.empty:
            daily = (
                base.groupby(base['invoicedate'].dt.normalize())['quantity']
                .sum()
                .reset_index()
            ...
            if len(daily) >= 5:
                ...
                future_dates = pd.date_range(
                    daily['invoicedate'].iloc[-1] + pd.Timedelta(days=1),
                    periods=horizon,
                    freq='D'
                )
                ...
                hist_plot = daily[daily['invoicedate'] >= cutoff][['invoicedate', 'quantity']]
                hist_plot = hist_plot.rename(columns={'quantity': 'value'})
                hist_plot['Series'] = 'Quantity'

                ma_plot = daily[daily['invoicedate'] >= cutoff][['invoicedate', 'MA7']]
                ma_plot = ma_plot.rename(columns={'MA7': 'value'})
                ma_plot['Series'] = 'MA7'

                fc = pd.DataFrame(
                    {'invoicedate': future_dates, 'value': y_pred, 'Series': 'Linear Forecast'}
                )
                ...
                fig = px.line(
                    ...
                    y='value',
                    color='Series',
                    labels={'invoicedate': 'Date', 'value': 'Units'},
                ...
                st.caption(
                    f"Model: y = {intercept:.2f} + {slope:.3f}·t   (t = day index). "
                    f"Note: simple linear trend; no seasonality."
                )
            else:
...
with tab_alerts:
    ...
    if 'stock_level' in df_filtered.columns and 'quantity' in df_filtered.columns:
        ...
        cols_to_show = [
            c for c in
            ['product', 'stock_level', 'quantity', 'projected_sales', 'remaining', 'status']
            if c in sim.columns
        ]
...
# ---------------- FOOTER ----------------
st.markdown(
    "<br><div style='text-align:center; color:#006565;'>"
    "Made with ♥ — Smart Inventory Analytics"
    "</div>",
    unsafe_allow_html=True
)
